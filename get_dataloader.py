...
class only_nsv_360_images_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # im = Image.open('../NSV_Processed_Data/Processed_360_JPEG_Images/Mahulpal-Jiral-R1/Romdas360/Mahulpal-Jiral-R1-Romdas360-0-00052.jpg')
...
class only_rdd_images_dataset(Dataset):
    def __init__(self, transform = None):
        self.path_list = []
        # for filename in os.listdir('../RDD2020_data/train/Czech/images/'):
        #     if filename[ - 4 : len(filename)] == '.jpg':
        #         self.path_list.append(f'../RDD2020_data/train/Czech/images/{filename}')
        for filename in os.listdir('../RDD2020_data/train/India/images/'):
            if filename[ - 4 : len(filename)] == '.jpg':
                self.path_list.append(f'../RDD2020_data/train/India/images/{filename}')
        # for filename in os.listdir('../RDD2020_data/train/Japan/images/'):
        #     if filename[ - 4 : len(filename)] == '.jpg':
        #         self.path_list.append(f'../RDD2020_data/train/Japan/images/{filename}')
...
def get_rdd_dataloaders(batch_size, shuffle_data = True):
    dataset = only_rdd_images_dataset()
    train_dataset, test_dataset = torch.utils.data.random_split(dataset, [int(0.9*dataset.__len__()), dataset.__len__() - int(0.9*dataset.__len__())])
    train_dataloader = DataLoader(train_dataset, shuffle = shuffle_data, num_workers = 4, batch_size = batch_size, drop_last=True)
    test_dataloader = DataLoader(test_dataset, shuffle = shuffle_data, num_workers = 4, batch_size = batch_size, drop_last=True)
    return train_dataloader, test_dataloader
